...
const NAV_ITEMS = [
  ...
  { name: "Editorial", id: "editorial" }, // Optional
            ...
            <div className="relative flex items-center bg-[#0B0B0C]/80 backdrop-blur-xl border border-[#F3EFE6]/10 rounded-full p-1.5 shadow-2xl shadow-black/80">
                ...
                <div className="hidden md:flex items-center gap-3 px-4 border-r border-[#F3EFE6]/10 mr-1 h-8">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-mono text-[#F3EFE6]/60 tracking-widest">SYS.ONLINE</span>
                    </div>
                ...
                {/* CENTER: NAVIGATION (THE LIQUID SOUL) */}
                <ul className="flex items-center relative z-10" onMouseLeave={() => setHoveredTab(null)}>
                    {NAV_ITEMS.map((item) => (
                        <NavItem
                            key={item.id}
                            item={item}
                            ...
                            isAnyHovered={!!hoveredTab} // Pass if *anything* is hovered
                ...
                {/* RIGHT: CONNECT UPLINK */}
                <div className="hidden md:flex items-center pl-1 ml-1 border-l border-[#F3EFE6]/10 h-8">
                    <a href="#contact" className="group/btn relative flex items-center gap-2 px-5 py-2 rounded-full hover:bg-white/5 transition-colors">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[#F3EFE6]">Connect</span>
                ...
                <div className="md:hidden px-2">
                    <button onClick={() => setIsMobileOpen(true)} className="p-2 text-[#F3EFE6] hover:bg-white/10 rounded-full transition-colors">
                        <Menu size={20} />
                    </button>
                </div>

                {/* BOTTOM: FUEL GAUGE (SCROLL PROGRESS) */}
                <div className="absolute bottom-0 left-6 right-6 h-[1px] bg-[#F3EFE6]/5 overflow-hidden rounded-full">
                    <motion.div style={{ width }} className="h-full bg-[#F3EFE6]/50 shadow-[0_0_10px_#F3EFE6]" />
                </div>
            </div>
        </div>
      </motion.nav>

      {/* --- MOBILE OVERRIDE MENU --- */}
      <AnimatePresence>
        {isMobileOpen && (
          <motion.div
          ...
          >
            ...
            <button onClick={() => setIsMobileOpen(false)} className="absolute top-8 right-8 p-4 text-[#F3EFE6] bg-white/5 rounded-full hover:rotate-90 transition-transform">
                <X size={24} />
            </button>
            ...
            <div className="absolute bottom-12 text-[#F3EFE6]/30 font-mono text-xs uppercase tracking-[0.2em]">
...
// --- SUBCOMPONENT: THE NAV ITEM & LIQUID PILL ---
function NavItem({ item, isActive, isHovered, isAnyHovered, onHover, onClick }) {
  
  // LOGIC: The pill should show if (I am Hovered) OR (I am Active AND Nothing else is hovered)
  ...
  return (
    <li className="relative list-none">
      ...
      >
        {/* THE SINGLE LIQUID SOUL (LayoutId makes it slide) */}
        {showPill && (
            <motion.div
                layoutId="nav-pill"
        ...
        {/* TEXT DECRYPTION LAYER */}
        <span className={`relative z-10 transition-colors duration-200 ${showPill ? "text-[#0B0B0C]" : "text-[#F3EFE6]"}`}>
    ...
    if (trigger) {
        ...
        intervalRef.current = setInterval(() => {
            ...
            if (iteration >= text.length) {
                clearInterval(intervalRef.current);
                setDisplayText(text); // Ensure clean end
            }
